...
import { 
  RouteBuilder, StoreSdk, IAppProjectProxyInit,
  IApiError, IProjectExecutionLog, AppProject, AppProjectKind, IProxyResult, 
...
describe('HTTP proxy', () => {
  describe('AppProjectProxy', () => {
    ...
    before(async () => {
      ...
      const p1 = AppProject.fromName('p1');
      p1.addRequest(`${apiUri}${RouteBuilder.backend()}`);
      await sdk.app.projects.create(p1.toJSON(), appId);
      pid = p1.key;
    });
    ...
    it('returns an error when no appId', async () => {
      const body = JSON.stringify({
        kind: AppProjectKind,
        pid,
        options: {},
      } as IAppProjectProxyInit);
      ...
      assert.equal(response.detail, 'The "appId" parameter is required.', 'has the detail');
    });
