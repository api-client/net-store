import { Permission, IPermission, IFile, IAccessAddOperation, IAccessRemoveOperation, PermissionRole, IGroup } from '@api-client/core';
import { Bytes } from 'leveldown';
import { SubStore } from './SubStore.js';
import { IPermissionStore } from './LevelStores.js';
 ...
 */
export class LevelPermissionStore extends SubStore implements IPermissionStore {
   ...
   */
  async addPermission(file: IFile, operation: IAccessAddOperation, addingUser: string, fn: () => IPermission, permission?: IPermission): Promise<string> {
    if (!permission) {
      permission = fn();
   ...
   */
  async addUserPermission(file: IFile, operation: IAccessAddOperation, addingUser: string): Promise<string> {
    const { id } = operation;
    if (!id) {
   ...
   * @returns The id of the created permission.
   */
  async addGroupPermission(file: IFile, operation: IAccessAddOperation, addingUser: string): Promise<string> {
    const { id } = operation;
    if (!id) {
   ...
   * @returns The id of the created permission.
   */
  async addAnyonePermission(file: IFile, operation: IAccessAddOperation, addingUser: string): Promise<string> {
